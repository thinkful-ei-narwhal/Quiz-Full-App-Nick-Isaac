...
function calculateCurrentQuestionNum(isQuestionPage = true) {
  return isQuestionPage
    ? `<p class='ques-number'>Question ${store.questionNumber + 1} out of ${
    store.questions.length
    }</p>`
    : `<p class='ques-number'>Question ${store.questionNumber} out of ${store.questions.length}</p>`;
}
...
function render(ans = null, lastQuestion = false) {
